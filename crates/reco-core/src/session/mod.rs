...
pub struct StitchSession {
    ...
    /// The 8 CUDA/Vulkan shared plane buffers, in the same slot order as
    /// `gpu_shared_textures`. These are copy sources only; rendering uses
    /// ordinary VramPool textures.
    #[cfg(target_os = "linux")]
